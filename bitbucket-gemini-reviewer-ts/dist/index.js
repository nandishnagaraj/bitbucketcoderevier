...
import { createCommitStatus, createInlinePullRequestComment, createPullRequestComment, getPullRequest, getPullRequestDiff } from "./bitbucket.js";
...
function isPullRequestEvent(req) {
...
function mapVerdictToBuildState(verdict) {
    return verdict === "fail" ? "FAILED" : "SUCCESSFUL";
}
function renderSummaryComment(prId, commitHash, review) {
...
function appendFindings(lines, heading, findings) {
...
function renderInlineComment(finding) {
    return [
...
async function runReview(input) {
    ...
    if (POST_INLINE_COMMENTS) {
        for (const finding of review.findings) {
            ...
            try {
                await createInlinePullRequestComment(input.workspace, input.repoSlug, input.prId, renderInlineComment(finding), finding.filePath, finding.line);
            }
            catch (error) {
    ...
    if (POST_BUILD_STATUS && pr.source?.commit?.hash) {
        await createCommitStatus(input.workspace, input.repoSlug, pr.source.commit.hash, mapVerdictToBuildState(review.summary.verdict), "ai-code-review", "AI Code Review", review.summary.headline, pr.links?.html?.href);
    }
    return {
        workspace: input.workspace,
        repoSlug: input.repoSlug,
